...
then
	...
	sed -Ei "s/MM_PUSHNOTIFICATIONHOST/$MATTERMOST_PUSH_PROXY_SERVICE_HOST/" $config
	sed -Ei "s/MM_PUSHNOTIFICATIONPORT/$MATTERMOST_PUSH_PROXY_SERVICE_PORT/" $config
	sed -Ei "s/MM_PUSHNOTIFICATIONCONTENTS/$MM_PUSHNOTIFICATIONCONTENTS/" $config
    echo OK
else
    echo SKIP
fi
